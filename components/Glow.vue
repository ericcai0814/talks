...
const distribution = computed(() => props.glow)
const opacity = computed<number>(() => +props.glowOpacity)
const hue = computed<number>(() => +props.glowHue)
const seed = computed<string>(() => {
  if (props.glowSeed === false || props.glowSeed === 'false')
    return Date.now().toString()
  return String(props.glowSeed || 'default')
})
...
<template>
  ...
  >
    <div
      class="clip bg-gradient-to-r from-[#1084d7] to-white/10"
      :style="{ 'clip-path': `polygon(${poly1})`, 'opacity': opacity }"
    />
    <div
      class="clip bg-gradient-to-l from-[#189196] to-white/10"
      :style="{ 'clip-path': `polygon(${poly2})`, 'opacity': opacity }"
    />
    <div
      class="clip bg-gradient-to-t from-[#d9c41e] to-white/10"
